 ...
 *
 * 1. Humans: `lark-to-codex secrets set/list/remove` to manage the
 *    encrypted keystore manually.
 *
 * 2. lark-cli (and any other tool implementing openclaw's exec-provider
 *    protocol): `lark-to-codex secrets get` reads a JSON-RPC request
...
export async function runSecretsSet(appId: string | undefined): Promise<void> {
  if (!appId) {
    console.error('用法: lark-to-codex secrets set --app-id <id>');
    process.exit(1);
  }
  const id = `app-${appId}`;
...
export async function runSecretsRemove(appId: string | undefined): Promise<void> {
  if (!appId) {
    console.error('用法: lark-to-codex secrets remove --app-id <id>');
    process.exit(1);
  }
  const id = `app-${appId}`;
